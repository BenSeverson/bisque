...
if [ "$persist_only" = "0" ]; then
    # ── Preflight ─────────────────────────────────────────────────────────
    # Every host a CI-identical install and build actually touches, and only
    # those. github.com carries the IDF sources and, per tools.json, almost
    # every tool archive; dl.espressif.com carries the Python dependency
    # constraints; the two components hosts are the component manager's
    # configured registry and storage URLs (IDF_COMPONENT_REGISTRY_URL /
    # IDF_COMPONENT_STORAGE_URL), which resolve idf_component.yml into
    # managed_components/.
    #
    # api.components.espressif.com is deliberately NOT listed. It appears in
    # idf_component_tools only as CANONICAL_IDF_COMPONENT_REGISTRY_API_URL, a
    # constant used to normalise registry URLs — a full `idf.py build`,
    # including fetching all six managed components, was verified to succeed
    # with that host firewalled off. Requiring it would fail the preflight on a
    # policy that is actually complete.
    ...
    if ! preflight_check "$PREFIX" \
        ...
        "components.espressif.com|http|https://components.espressif.com/|component registry — resolves idf_component.yml" \
        "components-file.espressif.com|http|https://components-file.espressif.com/|component archive downloads"; then
